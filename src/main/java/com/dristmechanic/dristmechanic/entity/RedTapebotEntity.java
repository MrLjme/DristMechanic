...
public class RedTapebotEntity extends Monster implements RangedAttackMob {
    ...
    public void performRangedAttack(LivingEntity target, float velocity) {
        ...
        this.level().playSound(null, this.getX(), this.getY(), this.getZ(),
                ModSounds.TAPEBOT_SHOOT.get(), SoundSource.HOSTILE, 1.0F, 1.0F);
    }
    ...
    protected @Nullable SoundEvent getAmbientSound() {
        return ModSounds.TAPEBOT_AMBIENT.get();
    }

    @Override
    protected @Nullable SoundEvent getHurtSound(@NotNull DamageSource damageSource) {
        return ModSounds.TAPEBOT_HURT.get();
    }

    @Override
    protected @Nullable SoundEvent getDeathSound() {
        return ModSounds.TAPEBOT_DEATH.get();
    }
