...
public class FarmManager {
    ...
    private static void checkFarmStability(ServerLevel level) {
        ...
        if (farmChunks == null || farmChunks.isEmpty()) {
            RaidManager.updateHolograms(level, List.of()); // Передаём пустой список вместо очистки
            return;
        }
        ...
        for (ChunkPos cp : farmChunks) {
            ...
            if (currentTick - maxLastChange >= 60) { // Стабильна 3 секунды
                Vec3 center = calculateExactCenter(level, connected);
                if (center != null)
                    stableFarms.add(new FarmData(connected, totalValue, center, findEdgeChunks(connected)));
            }
        }
        RaidManager.updateHolograms(level, stableFarms);
    }

    public record FarmData(List<ChunkPos> chunks, int totalValue, Vec3 center, List<ChunkPos> edgeChunks) {
        public boolean isEmpty() {
            return chunks.isEmpty() || totalValue == 0;
        }
    }
    ...
    public static FarmData findConnectedFarm(ServerLevel level, ChunkPos startChunk) {
        ...
        if (!fC.contains(startChunk)) return new FarmData(List.of(), 0, null, List.of());
        ...
        for (ChunkPos cp : connected) {
            ...
            if (c != null) tV += c.getData(ModAttachments.CROP_COUNT.get());
        }
        return new FarmData(connected, tV, calculateExactCenter(level, connected), findEdgeChunks(connected));
    }
